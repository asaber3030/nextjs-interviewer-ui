      ...
      <DialogContent className="bg-white">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleUpdate)} className="space-y-2">
            <SelectField control={form.control} name="planId" label="Plan" defaultValue={String(user.planId)} valueAsNumber>
              {plans.map((plan) => (
                <SelectItem value={String(plan.id)}>{plan.name}</SelectItem>
              ))}
            </SelectField>

            <SelectField control={form.control} defaultValue={String(user.careerId)} name="careerId" label="Career" valueAsNumber>
              {careers.map((career) => (
